...
if generate_button:
    ...
    # print("\n\n\n\nThis is the result\n\n\n\n\n", str(result))
    ques, context = process_question(user_input)
    coder = agents_func()
    task2 = tasks_func(ques, d2_examples, diagram_language, coder)
    final_result = crew_func(coder, task2)
    print("\n\n\n\nThis is the result\n\n\n\n\n", str(final_result))


    pattern = r"```([\s\S]*?)```"
    match = re.search(pattern, str(final_result))
    if match:
